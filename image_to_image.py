...
@app.cls(
    image=image, gpu="A100", volumes=volumes,scaledown_window=240
)
class Model:
    @modal.enter()
    def enter(self):
...
image_name = "vijay.jpg"

@app.local_entrypoint()
def main(
    image_path=f"/home/bhirahateesvaran/diffusions/images/{image_name}",
    prompt="Photo of male model, black and white, realistic, portrait, not painted, very detailed faces, skin with pores, 4k, dark suit, black plain background, skin not smoothed, face from left to right, desire in the eyes, half profile, glowering, looking into the camera, dark shirt.",
    ...
    dir = Path(f"{os.getcwd()}/images/output/image-2-image")
    dir.mkdir(exist_ok=True, parents=True)

    characters = string.ascii_letters + string.digits
    name = ''.join(secrets.choice(characters) for _ in range(15))
    
    output_path = dir / f"output_{name}.png"
